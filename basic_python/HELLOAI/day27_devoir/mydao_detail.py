...
from day27_devoir.mylog import MyLog
...
class MyDao_Detail:
    ...
    def myselect(self,survey_id):
        ...
        for name in self.cursor:        
            list.append( {'survey_id':name[0],'s_seq':name[1], 'question':name[2], 'a1':name[3], 'a2':name[4], 'a3':name[5], 'a4':name[6], 'in_date':name[7], 'in_user_id':name[8], 'up_date':name[9], 'up_user_id':name[10],'up_user_name':name[11] })
        
        return list
    
        
    def myinsert(self, survey_id, s_seq, question, a1, a2, a3, a4, in_user_id, up_user_id): 
        sql_str = mybatis_mapper2sql.get_child_statement(self.mapper, "detail_insert")    
        MyLog().getLogger().debug(sql_str)

        self.cursor.execute(sql_str,(survey_id, s_seq, question, a1, a2, a3, a4, in_user_id, up_user_id))
